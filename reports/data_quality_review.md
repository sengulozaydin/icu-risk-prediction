...
- Notebook 03: existing negative urine exclusion moved upstream so the first saved table is already correct. The 555,975 mL raw entry was removed by the explicit gross-error screen (>100,000 mL per urine entry); The affected stay now totals 5,215 mL; the other corrected stay remains at 1,200 mL. EBL and chest-tube flags are byte-value-equivalent to their previous table columns.
...
424,942 missing cells: lab 245,988; vital 176,204; urine 2,750. There are 40,043 rows with at least one missing feature; all remain. Per-column counts are in [data_quality_summary.json](data_quality_summary.json). Lab/vital missing-table indicators retain their original row-presence semantics. No individual NaN imputation or category encoding.

## Open clinical review items

Structural sanity checks passed; this is not a claim that every retained extreme is valid. Feature-specific audit CSVs are generated locally and excluded from version control. Tiny positive vital readings, temperatures outside 26–45 C, unusually high lab endpoints and 9 urine totals above 20 L/day remain flagged. The maximum retained urine total is 98,280 mL and comprises multiple high source records; verify measurement/irrigation context before deciding on exclusion. Do not infer a corrected value. EBL max 50,000 mL and present-row p99/p99.5/p99.9 of 10,000/13,685/25,000 mL remain unchanged.
...
Cohort, intervention, demographics, raw vital cache and unrelated notebooks were checked unchanged by SHA-256.
